# Build a Boat building tools

> [!CAUTION]
> The app is not downloadable on any other website. Only this GitHub Repository. Any other websites that offer downloads to this app are most likely scams and can harm your device.

Greetings! Welcome to my project, Build a Boat Building tools. This is a test version, don't expect a full-fledged and customizable C# app!
Underneath is a table of contents, feel feree to read!

---

## Table of contents
- Left sidebar with a main content section - displays the content
- Window control buttons: X to close, - to minimize
- Polygon section: Calculates the dimensions required for a polygon with the desired number of sides and radius!
- Bridging blocks section: Bridge blocks (Connect blocks) with any X and Y coordinate
- Pyramid section: Create pyramids with any radius, height, and number of sides
- Settings section (Work In Progress!)

---

## The app's purpose
The purpose of this app is to aid other builders in creating polygons, pyramids, or bridge blocks.
It's basically a third-party build "helper".

## Approximate size of the app?
The approximate size of this app is around 133 megabytes.

### Is it safe?
**Yes**, this app is completely safe and free to use. The app **does not contain malware** and is **not malicious**.

### Who created this?
Kane, or hmehfg (me), created this app, no one else.
This is a non-profit project!

## Where can I download it?
This app is only available through this GitHub repository.
Download it here -> 

*Developed for educational purposes. Built with passion for builders.*
